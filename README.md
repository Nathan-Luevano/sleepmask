...
| `tests/` | the 11-layer validation matrix and the Unicorn harnesses (PEB-walk environment, XNU syscall-class emulator) |
...
bash tests/test_all.sh   # the 11-layer matrix, one command
```
...
- **windows-dll** — `tools/mk_dll.py` wraps the 1507-byte PIC beacon in a
  ...
  beacon **twice** (an NtWriteFile on stdout AND an NtCreateFile/NtWriteFile/
  NtClose of `sleepmask_beacon.txt`, the filename decoded from live memory),
  R15 sentinel preserved, under real and decoy syscall nr.
...
- **windows-real** — the same SAC-proof entry mode, but the blob is the
  **real 1212-byte sleepmask payload** (not the beacon): `call`-entered at four
  ...
  `run-shell.ps1` exercises on real hardware.
